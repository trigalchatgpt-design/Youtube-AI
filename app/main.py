...
app = FastAPI(title="YouTube AI Studio", version="0.8.0")
...
def _run_hubble_episode_once():
    try:
        result = run_hubble_private_episode()
        print(f"HUBBLE_RESULT {result}", flush=True)
    except Exception as exc:
        print(f"HUBBLE_ERROR {type(exc).__name__}: {exc}", flush=True)


def _run_batch24_once():
    try:
        results = run_batch24()
        print(f"BATCH24_RESULT {results}", flush=True)
    except Exception as exc:
        print(f"BATCH24_ERROR {type(exc).__name__}: {exc}", flush=True)
...
def _startup():
    ...
    if os.getenv("RUN_HUBBLE_PRIVATE_EPISODE", "").strip().lower() in {"1", "true", "yes", "on"}:
        threading.Thread(target=_run_hubble_episode_once, daemon=True).start()
    if os.getenv("RUN_BATCH24", "").strip().lower() in {"1", "true", "yes", "on"}:
...
@app.get("/health")
def health():
    return {"ok": True, "service": "youtube-ai-studio", "version": "0.8.0"}
